...
var allButtons = [
    ...
    ['<span title="Чтобы поместить в текст химическую формулу, поставьте курсор в нужное место и нажмите на эту кнопку. Появятся открывающий и закрывающий теги <chem> ... </chem>. Между ними впишите химическую формулу. Не вставляйте внутрь тегов слова — только формулы">chem</span>','<chem>%s</chem>', is_chem],
    ...
    ['<span title="Двойная связь">O=C</span>', '\\rtrigonal{0==C;1D==O;}', is_chem],
...
prepare_editor = function(){
    ...
    $('.math_editor').each(function(index,elem){
        ...
        special[6] = $('<div class="math_ed_butt">...</div>').click(function(){
            ...
            return;
            let all_btn = $('<span></span>');
            for (inum in allButtons){
                var item = allButtons[inum];
                all_btn.append($('<div class="math_ed_butt">').html('<span>'+item[0]+'</span>').data('tpl', item[1]).data('ta', elem).click(function(){
        ...
        special[2] = $('<div class="math_ed_butt" title="Вставка таблицы">Табл</div>').data('ta', ta).click(function(){
            ...
            /*var insert_match = function(){
                var html = '\n\n<table style="width:600px;margin:auto;border:0"><tr><td style="text-align:center;vertical-align:middle;border:0;width:282px">ПОНЯТИЕ 1 </td> <td style="border:0;width:36px">&#8239;</td> <td style="text-align:center;vertical-align:middle;border:0">ПОНЯТИЕ 2</td></tr><tr><td style="text-align:left;vertical-align:top;border:0"> Список 1 </td> <td style="border:0"> &#8239; </td> <td style="text-align:left;vertical-align:top;border:0"> Список 2 </td></tr></table>\n\n'
                insertAtCaret($(t).data('ta')[0], html);
                jq_preview($(t).data('ta').attr('id').replace('editor_', ''), true);
                main_div.remove();
            };*/
